...
function ClassesView({ classes, onCapture, onPortfolio }: { classes: LiveClass[]; onCapture: (item: LiveClass) => void; onPortfolio: (item: LiveClass) => void }) {
  return <div className="live-page"><div className="live-heading"><div><span className="eyebrow-orange">CLASSES</span><h1>Your classes.</h1><p>Open a class to capture evidence or review its Sportfolio.</p></div><a className="setup-inline" href="/live/setup">Set up classes →</a></div><div className="class-list">{classes.map((item) => <article key={item.id} className="class-card"><div><span>{item.activity ?? "PE"}</span><h2>{item.name}</h2><p>{item.academic_year} · {item.pupil_count ?? 0} pupils</p></div><div><button onClick={() => onCapture(item)} disabled={(item.pupil_count ?? 0) === 0}>{(item.pupil_count ?? 0) === 0 ? "Add pupils first" : "Capture"}</button><button onClick={() => onPortfolio(item)}>Sportfolio</button></div></article>)}</div></div>;
}
